<!-- 实现思路（将购物车选中列表所有信息传递（不经过SSM后端），或者传递选中列表的id数组（到达提交订单页面需要根据id数组查询购物车列表）（经过SSM后端））
为了避免进入提交订单页面刷新之后在购物车选中的列表数据消失，建议将购物车选中列表所有信息存储到localstorage（仿照存储token），以下过程中没有实现，完成以下步骤后自己可以优化此步骤

 -->
<template>
  <div>
    <el-table :data="tableData" tooltip-effect="dark" style="margin:50px auto;width: 70%">
